...
/**
 * Servlet implementation class FriendLocationListServlet
 */
@WebServlet("/FriendLocationListServlet")
public class FriendLocationListServlet extends HttpServlet {
...
//		if(!checkParameter(response, "id", id, root, pw)) return;
		...
		try {
			list_tFriends = dao.getByQuery("user_id=" + user_id + " and show_location=1", 0, 0);
			...
			for(int i=0; i<list_tFriends.size(); i++){
				...
				friend.address = userDao.getById(list_tFriends.get(i).getFriend().getId()).getAddress();
