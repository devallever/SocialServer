...
/**
 * Servlet implementation class VisitedForUserServlet
 */
@WebServlet("/VisitedForUserServlet")
public class VisitedForUserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<User> list_visited_user_user = new ArrayList<User>();
		User user;
		try {
			list_tuser_visited = dao.getByQuery("user_id=" + user_id + "order by date desc", 0, 0);
			...
			for(int i=startCount-1; i<(startCount-1+10); i++){
				if(i<list_visited_user_user.size()) list_user_for_client.add(list_visited_user_user.get(i));
				
			}
		
			
			root.visiteduser_list = list_user_for_client;
			root.success = true;
			root.message = "";
			dao.close();
			pw.print(gson.toJson(root));
