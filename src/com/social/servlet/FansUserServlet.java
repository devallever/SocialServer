...
import com.social.servlet.FollowUserServlet.Root;
import com.social.servlet.FollowUserServlet.User;
import com.social.util.CommonUtils;

/**
 * Servlet implementation class FansUserServlet
 */
@WebServlet("/FansUserServlet")
public class FansUserServlet extends HttpServlet {
...
//		}
		...
		try {
			...
			list_fans = dao.getByQuery("follow_id=" + id + "order by id desc", 0, 0);
			for(TFollow tfollow: list_fans){
				tfans_user = tfollow.getUser();
				user = new User();
				user.id = tfans_user.getId()+"";
				user.username = tfans_user.getUsername();
				user.nickname = tfans_user.getNickname();
				user.user_head_path = tfans_user.getHeadpath();
				user.age = tfans_user.getAge();
				user.sex = tfans_user.getSex();
				user.signature = tfans_user.getSignature();
				user.occupation = tfans_user.getOccupation();
				double distance = CommonUtils.getDistance(str_latitude, str_longitude, String.valueOf(tfans_user.getLatitude()), String.valueOf(tfans_user.getLongitude()));
				String dis_with_two = new java.text.DecimalFormat("#.00").format(distance);
				user.distance = Double.parseDouble(dis_with_two);
				user.is_vip = tfans_user.getIs_vip();
				list_user.add(user);
			}
			...
			root.success = true;
			root.message = "";
			root.list_fans_user = list_user_for_client;
